...
| `teacher-report.yaml` | a session's note reaches the practice report: the share chooser on Home, the week/month toggle, the preview, Share opening the OS sheet, and the piece page's single-piece report with its empty state (the aggregation is `check:report`) |
