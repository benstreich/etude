...
| `suggested-session.yaml` | history seeded via log-past (`scripts/stale-days.js`) makes the Practice picker suggest a session; Start reaches the runner, dismiss hides the card and the dismissal survives a restart (the composer is `check:suggest`) |
